          ...
          onChunk: (chunk: RunResponse) => {
            if (
              ...
              setMessages((prevMessages) => {
                ...
                if (chunkAny.metadata && typeof chunkAny.metadata === 'object' && chunkAny.metadata.event) {
                  const innerChunk = chunkAny.metadata;
                  ...
                  if (innerChunk.metadata && Object.keys(innerChunk.metadata).length > 0) {
                    const existingMeta = (lastSubMessage as any).metadata || {};
                    (lastSubMessage as any).metadata = { ...existingMeta, ...innerChunk.metadata };
                    // We can also trigger the global state here if it's currently selected, but usually click-to-view is better.
                    // Wait, the user wants Right Sidebar to show metadata. If they click nothing, should it open automatically?
                    // The sidebar opens if `selectedMetadata` is set.
                    // Let's set it in a safe way if the user explicitly wants to see it, but we can't hook call inside setMessages directly easily.
                    // We just attach it to `lastSubMessage.metadata`.
                  }
                  ...
                  if (
                    ...
                    lastSubMessage.tool_calls = processChunkToolCalls(innerChunk, lastSubMessage.tool_calls)
                  } else if (
                    ...
                    const incomingSteps = innerChunk.extra_data?.reasoning_steps ?? []
                    lastSubMessage.extra_data = {
                      ...lastSubMessage.extra_data,
