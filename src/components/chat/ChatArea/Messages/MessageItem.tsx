...
import { AgentMessageWrapper } from './Messages'
    ...
    messageContent = (
      <div className="flex w-full flex-col gap-4">
        ...
        {message.sub_messages && message.sub_messages.length > 0 && (
          <div className="flex w-full flex-col gap-4 pl-4 border-l-2 border-border/40 mt-2 bg-muted/10 p-4 rounded-r-lg">
            {message.sub_messages.map((subMsg, idx) => (
              <AgentMessageWrapper key={`sub-${idx}`} message={subMsg} isLastMessage={false} />
            ))}
          </div>
        )}
      </div>
